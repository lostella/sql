...
public class PPLQueryFormatterTest {
  ...
  public void testFieldsCommand() {
    assertFormatting(
        "source=logs|fields name,age,status", "source=logs | fields name, age, status");
  }
  ...
  public void testFieldsWithMinusPrefix() {
    assertFormatting(
        "source=logs|fields - password,secret", "source=logs | fields - password, secret");
  }
  ...
  public void testWhereWithLogicalNot() {
    assertFormatting(
        "source=logs|where NOT status='inactive'", "source=logs | where not status = 'inactive'");
  }
  ...
  public void testEvalCommand() {
    assertFormatting("source=logs|eval doubled=value*2", "source=logs | eval doubled = value * 2");
  }
  ...
  public void testEvalWithArithmetic() {
    assertFormatting(
        "source=logs|eval result=(a+b)*c/d", "source=logs | eval result = (a + b) * c / d");
  }

  @Test
  public void testEvalWithFunction() {
    assertFormatting(
        "source=logs|eval upper_name=upper(name)", "source=logs | eval upper_name = upper(name)");
  }
  ...
  public void testStatsCommand() {
    assertFormatting(
        "source=logs|stats count() by status", "source=logs | stats count() by status");
  }
  ...
  public void testStatsWithAllnumArg() {
    assertFormatting(
        "source=logs|stats ALLNUM=true count()", "source=logs | stats allnum=true count()");
  }

  @Test
  public void testStatsWithDelimArg() {
    assertFormatting(
        "source=logs|stats DELIM=',' count()", "source=logs | stats delim=',' count()");
  }
  ...
  public void testStatsDistinctCount() {
    assertFormatting(
        "source=logs|stats DISTINCT_COUNT(user_id)", "source=logs | stats distinct_count(user_id)");
  }
  ...
  public void testEventstatsCommand() {
    assertFormatting(
        "source=logs|eventstats count() by status", "source=logs | eventstats count() by status");
  }
  ...
  public void testDedupMultipleFields() {
    assertFormatting(
        "source=logs|dedup user_id,session_id", "source=logs | dedup user_id, session_id");
  }
  ...
  public void testDedupWithKeepempty() {
    assertFormatting(
        "source=logs|dedup user_id keepempty=true", "source=logs | dedup user_id keepempty=true");
  }
  ...
  public void testRenameCommand() {
    assertFormatting(
        "source=logs|rename old_name as new_name", "source=logs | rename old_name as new_name");
  }
  ...
  public void testRenameUppercase() {
    assertFormatting(
        "source=logs|RENAME oldField AS newField", "source=logs | rename oldField as newField");
  }
  ...
  public void testRareMultipleFields() {
    assertFormatting(
        "source=logs|rare status,method by host", "source=logs | rare status, method by host");
  }
  ...
  public void testGrokUppercase() {
    assertFormatting(
        "source=logs|GROK message '%{IP:client}'", "source=logs | grok message '%{IP:client}'");
  }
  ...
  public void testFillnullCommand() {
    assertFormatting(
        "source=logs|fillnull value=0 field1", "source=logs | fillnull value=0 field1");
  }
  ...
  public void testFillnullUppercase() {
    assertFormatting("source=logs|FILLNULL value=0 count", "source=logs | fillnull value=0 count");
  }
  ...
  public void testTrendlineCommand() {
    assertFormatting("source=logs|trendline sma(3,value)", "source=logs | trendline sma(3, value)");
  }
  ...
  public void testBinCommand() {
    assertFormatting("source=logs|bin span=1h timestamp", "source=logs | bin span=1h timestamp");
  }

  @Test
  public void testBinWithBins() {
    assertFormatting("source=logs|bin bins=10 value", "source=logs | bin bins=10 value");
  }

  @Test
  public void testBinUppercase() {
    assertFormatting("source=logs|BIN SPAN=1d timestamp", "source=logs | bin span=1d timestamp");
  }
  ...
  public void testLowerFunction() {
    assertFormatting(
        "source=logs|eval lower_name=LOWER(name)", "source=logs | eval lower_name = lower(name)");
  }
  ...
  public void testBooleanLiteralInStatsArgs() {
    ...
    assertFormatting(
        "source=logs|stats allnum=TRUE count()", "source=logs | stats allnum=true count()");
    assertFormatting(
        "source=logs|stats allnum=FALSE count()", "source=logs | stats allnum=false count()");
  }
  ...
  public void testPipeSpacing() {
    assertFormatting("source=logs|where a=1|where b=2", "source=logs | where a = 1 | where b = 2");
  }
  ...
  public void testMultiplicationNumberFirst() {
    assertFormatting("source=logs|eval result=2*value", "source=logs | eval result = 2 * value");
  }
  ...
  public void testChainedArithmetic() {
    assertFormatting(
        "source=logs|eval complex=a*2+b/3-c", "source=logs | eval complex = a * 2 + b / 3 - c");
  }
  ...
  public void testDottedFieldName() {
    assertFormatting(
        "source=logs|fields user.name,user.email", "source=logs | fields user.name, user.email");
  }

  @Test
  public void testNestedFieldAccess() {
    assertFormatting(
        "source=logs|where user.profile.age>30", "source=logs | where user.profile.age > 30");
  }
  ...
  public void testPreserveStringLiteralContent() {
    ...
    assertFormatting(
        "source=logs|where message='Hello World'", "source=logs | where message = 'Hello World'");
  }
  ...
  public void testASTEquivalenceMultiplePipes() {
    assertASTEquivalence("source=logs | where a = 1 | where b = 2 | where c = 3 | fields a, b, c");
  }
