...
public class PPLQueryFormatterTest {
  ...
  public void testEvalCommand() {
    assertFormatting(
        "source=logs|eval doubled=value * 2", "source=logs | eval doubled = value * 2");
  }
  ...
  public void testBinCommand() {
    ...
    assertFormatting("source=logs|bin timestamp span=1h", "source=logs | bin timestamp span=1h");
  }
  ...
  public void testBinUppercase() {
    // Grammar: BIN fieldExpression binOption* - field comes first, then options
    assertFormatting("source=logs|BIN timestamp SPAN=1d", "source=logs | bin timestamp span=1d");
  }
