 ...
 *   <li>Command formatting (fields, where, stats, eval, join, sort, head, dedup, rename, etc.)</li>
 ...
 *   <li>AST equivalence (original and formatted queries parse to equivalent ASTs)</li>
 * </ul>
 */
public class PPLQueryFormatterTest {
    ...
    /**
     * Assert that the formatted query parses successfully and produces an equivalent AST.
     * This verifies semantic preservation.
     */
    ...
    /**
     * Compare two parse trees for semantic equivalence.
     * Ignores whitespace differences and keyword case.
     */
    ...
    /**
     * Compare children flexibly, allowing for certain structural differences
     * that don't affect semantics (e.g., optional SEARCH keyword).
     */
    ...
    public void testBinCommand() {
        assertFormatting(
            "source=logs|bin span=1h timestamp",
            "source=logs | bin span=1h timestamp"
        );
    }
    ...
    public void testBinUppercase() {
        assertFormatting(
            "source=logs|BIN SPAN=1d timestamp",
            "source=logs | bin span=1d timestamp"
        );
    }
    ...
    public void testASTEquivalenceComplexQueries() {
        ...
        assertASTEquivalence(
            "source=logs | eval doubled = value * 2 | where doubled > 100 | stats avg(doubled) by category"
        );
    }
